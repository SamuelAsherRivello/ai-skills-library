...
3. Inspect Git status and the skill diff. Stop for unrelated local changes or a non-fast-forward remote state; never overwrite, rebase, force-push, or discard changes.
4. Copy the source skill directory into this library's `.agents/skills` directory. Review the resulting diff, run the repository's applicable skill validation, then commit only the named skill's files with a descriptive message.
